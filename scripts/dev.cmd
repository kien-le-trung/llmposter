...
start "LLMposter Backend" cmd /k "cd /d ""%BACKEND_DIR%"" && call venv\Scripts\activate.bat && set APP_ENV=development&& set BACKEND_CORS_ORIGINS=http://localhost:3000&& set DATABASE_URL=postgresql+psycopg://postgres:postgres@localhost:5432/llmposter&& set MODEL_SERVER_URL=http://localhost:11434&& set MODEL_NAME=qwen2.5:0.5b&& set INFERENCE_MODE=remote&& set AGENT_CONFIG_SOURCE=database&& set WORD_SELECTION_MODE=random&& python -m uvicorn app.main:app --reload --host 0.0.0.0 --port 8000"
